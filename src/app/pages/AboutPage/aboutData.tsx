import background1 from '../../../assets/img/about-us_vacancy-1.jpg';
import background2 from '../../../assets/img/about-us_faq-2.jpg';
import background3 from '../../../assets/img/about-us_careers-content-bg-3.jpg';
import background4 from '../../../assets/img/about-us_careers-content-bg-4.jpg';

export const aboutData = [
	{
		title: 'Don’t see a courses that fits you?',
		text: `Send us your CV anyway! We’re always starting new projects and we’d love to work with you. We’ll keep your CV on file and get in touch as soon as we have a good match for you!`,
		background: background1,
		linkText: 'Send your CV right',
	},
	{
		title: 'Equal Employment Opportunity (EEO)',
		text: `Exadel is fully committed to attracting, retaining, developing, and promoting the most qualified employees without regard to their race, color, national origin, religion, gender, age, disability, genetic information, reprisal, sexual orientation, family status, citizenship status, veteran status, or any other characteristic specified by state or local law. We are dedicated to providing a work environment in which employees are treated with respect and dignity, free from discrimination and harassment.`,
		background: background2,
	},
	{
		title: 'Our Internship Program',
		text: `Qualified students at the Belarusian State University in Minsk have been working with Exadel for more than half a decade to access unparalleled learning opportunities and potentially start their long-term career.\n
    
    In our competitive internship program, about 25% of each class will win their first professional position with us upon completing student work. We welcome students in technology-related majors that excel in academics to pursue internship opportunities with Exadel.\n
    
    Here, as part of our development team, students work closely with experienced engineers to quickly deepen their skills through real-world work in coding, development, design, and testing. Being part of a consultancy organization, a strong emphasis on teamwork is also part of every internship experience. Interested in learning more?`,
		background: background3,
	},
	{
		title: 'Frequently Asked Questions',
		text: `Applying for jobs can be confusing and difficult. At Exadel, we strive to be as open and transparent with our current and future employees as possible, and that starts in the hiring process. The following are some questions that candidates frequently ask us, as well as our answers, so you know exactly what to expect.`,
		background: background4,
		linkText: 'Read FAQ',
	},
];
